...
Let's take an example with one of my packages: [`tidypolars`](https://tidypolars.etiennebacher.com/) (I won't spend time explaining each part of the file, refer to the [Adding rules](https://flir.etiennebacher.com/articles/adding_rules) vignette for more details).

## State of the package

At the time of writing, `tidypolars` (0.13.0) contains several deprecated functions, such as `describe()` and `TODO`. Those functions should be replaced by `summary()` and `describe_plan()` respectively.
...
Note that those rules are bundled with the package, meaning that if the package developers change or remove some rules and you update the package, the rules you will have access to will also be different.
