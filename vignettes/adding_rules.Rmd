...
`cli::cli_abort()`, for instance because we prefer the formatting of the output
with the latter.
```{r, error=TRUE}
...
This is not possible with `cli::cli_abort()`, which needs everything to be in the
argument `message`, meaning that we need to manually put all those elements in
`paste0()`:

```{r, error=TRUE}
n <- 10
cli::cli_abort(paste0("Got ", n, " values instead of 1."))
```

Therefore, we can look for the pattern `stop(...)` and replace it by
`cli::cli_abort(paste0(...))`. Capturing all elements in a pattern is done with
...
rule:
  pattern: stop($$$ELEMS)
fix: cli::cli_abort(paste0(~~ELEMS~~))
message: Use `cli::cli_abort()` instead of `stop()`.
```
...
#> Suggestion: Use `cli::cli_abort()` instead of `stop()`.
...
#> New code: cli::cli_abort(paste0("Got ", n, " values instead of 1."))
```
